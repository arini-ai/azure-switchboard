...
class MockAzureOpenAI:
    """Mock implementation of AsyncAzureOpenAI for testing"""

    def __init__(self, *args, **kwargs):
        self.models = AsyncMock()
        self.models.list = AsyncMock(return_value=["gpt-4"])

        # Create mock chat completions client
        self.chat = MagicMock()
        self.chat.completions = AsyncMock()
        self.chat.completions.create = AsyncMock()

    async def set_unhealthy(self):
        """Make this client unhealthy by raising an exception on models.list"""
        self.models.list.side_effect = Exception("Service unavailable")

    async def set_healthy(self):
        """Make this client healthy again"""
        self.models.list.side_effect = None


def mock_client_factory(config: AzureDeployment) -> MockAzureOpenAI:
    """Factory that creates mock clients for testing"""
    ...
    # Configure the chat.completions.create mock to return useful response
...
def deployment_configs():
    ...
    return [
        AzureDeployment(
            endpoint="https://test1.openai.azure.com/",
            api_key="key1",
            deployment_name="gpt-4-test1",
            tpm_limit=100000,
            rpm_limit=1000,
        ),
        AzureDeployment(
...
def test_switchboard_init(deployment_configs):
    ...
    assert all(isinstance(d.client, MockAzureOpenAI) for d in sb.deployments)
